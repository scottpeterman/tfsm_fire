...
def find_database(db_path: Optional[str] = None) -> Optional[Path]:
    ...
    cwd_db = Path.cwd() / "oui.db"  # adjust filename as needed
...
try:
    ...
    try:
        ...
        try:
            ...
            from tfire import TextFSMAutoEngine
        ...
        try:
            ...
            for i, template in enumerate(self.templates_to_download, 1):
                ...
                try:
                    ...
                    stats['errors'] += 1
                    self.progress.emit(i, total, f"X {cli_command}: {str(e)[:30]}")
...
class TemplateTestWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Now returns 4 values: best_template, best_parsed, best_score, all_scores
            best_template, best_parsed, best_score, all_scores = engine.find_best_template(
                self.device_output, self.filter_string
            )

            # Get template content separately from DB
            ...
            self.results_ready.emit(
                best_template or "None",  # str
                best_parsed or [],  # list
                best_score,  # float
                all_scores,  # list (of tuples)
                template_content or ""  # str
            )
        except Exception as e:
            traceback.print_exc()
...
Start
  ^${NEIGHBOR}\\s+${LOCAL_INTERFACE}\\s+\\d+\\s+\\S+\\s+${NEIGHBOR_INTERFACE} -> Record""")
    ...
    def handle_db_results(self, best_template: str, best_parsed: list, best_score: float,
        ...
        # Update all templates table with scores
        self.all_templates_table.setRowCount(len(all_scores))
        for row, (name, score, records) in enumerate(all_scores):
            self.all_templates_table.setItem(row, 0, QTableWidgetItem(name))
            self.all_templates_table.setItem(row, 1, QTableWidgetItem(f"{score:.2f}"))
            self.all_templates_table.setItem(row, 2, QTableWidgetItem(str(records)))
            self.all_templates_table.setItem(row, 3, QTableWidgetItem("Match"))
    ...
    def log_db_results(self, best_template: str, best_parsed: list, best_score: float, all_scores: list):
        ...
        log.append(f"Templates Matched: {len(all_scores)}")
        ...
        log.append("-" * 40)
        for name, score, records in all_scores:
            log.append(f"• {name}: Score={score:.2f}, Records={records}")
